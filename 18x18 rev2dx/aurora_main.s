...
;	1.5	random animation patterns added
;===============================================================================
...
mode_08:		;-------------------------------------------
				...
				mov		#handle(random_anim),w0		; set preprocessor
				mov		w0,preprocess
...
mode_09:		;-------------------------------------------
				...
				mov		#handle(random_anim3),w0		; set preprocessor
				mov		w0,preprocess
...
random_anim3:	;-------------------------------------------
				...
				mov		#LED_data_R,w1				; w1 -> LED_data_R
				mov		#LED_data_G,w2				; w2 -> LED_data_R
				mov		#LED_data_B,w3				; w3 -> LED_data_R
				mov		#LED_data_R+num_LEDs*2,w9	; w9 -> end of LED_data_B
0:													; do {
				cp0.b	[w1]						; 	if ([w1] == 0)
				bra		NZ,1f						; 	
				cp0.b	[w2]						; 	and ([w2] == 0)
				...
				bra		NZ,1f						; 	{
				call	random_number				; 	  get a random number -> w0
				mov.b	w0,[w1]						; 	  random brightness
				bset	[w1],#DUTY_DIR				; 	  duty dir = down
				bclr	[w1],#DUTY_HOLD				; 	  release duty hold
				call	random_number				; 	  get a random number -> w0
				mov.b	w0,[w2]						; 	  random brightness
				bset	[w2],#DUTY_DIR				; 	  duty dir = down
				bclr	[w2],#DUTY_HOLD				; 	  release duty hold
				call	random_number				; 	  get a random number -> w0
				mov.b	w0,[w3]						; 	  random brightness
				bset	[w3],#DUTY_DIR				; 	  duty dir = down
				bclr	[w3],#DUTY_HOLD				; 	  release duty hold
